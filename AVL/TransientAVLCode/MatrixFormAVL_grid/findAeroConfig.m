function [aeroTriggers,aeroMatrix] = findAeroMatrix(settings,currentAeroConfig)
    success = 1;
    ...
    AeroMatrix = generateMatrix(allFound,settings);
...
function [theMatrix] = generateMatrix(structCoords,settings)
    i = structCoords(1);
    j = structCoords(2);
    k = structCoords(3);
    l = structCoords(4);
    ...
    syms a b de v p q r; % alph beta d_elev vel p q r
    ...
    EQN(2) = D == CD * Q * Sref;
    EQN(3) = Y == CY * Q * Sref;
    EQN(4) = l == Cl * Q * Sref * Bref;
    EQN(5) = m == Cm * Q * Sref * Cref;
    EQN(6) = n == Cn * Q * Sref * Bref;
    
    [theMatrix,theAnswers] = equationsToMatrix(EQN)
    vars = symvar(EQN)
end
