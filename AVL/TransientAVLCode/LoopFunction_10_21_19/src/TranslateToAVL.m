function [success] = TranslateToAVL(sail)
...
if isfield(sail, 'name') ~= 1
    ...
    runs.name = '.\AVL_output\AVL_output';
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
const.r = (sail.p1*runs.Bref) / (2*runs.vel);        % roll rate (p)
const.p = (sail.q1*runs.Cref) / (2*runs.vel);        % pitch rate (q)
const.y = (sail.r1*runs.Bref) / (2*runs.vel);        % yaw rate (r)
...
successFindStall = findStall_newclassic(design, 0);
if ~successFindStall
    disp("findSTall problem");
    success = 0;
    return;
end
