function cm_calc_saurabh_standalone(filename)
...
% check for AVL
if exist('avl.txt', 'file') == 2
    error('YODER: Must manually change avl.txt to avl.exe before running code!');
end
if exist('avl.exe', 'file') ~= 2
    error('YODER: You need to have avl.exe in this folder: %s', pwd);
end
% keyboard
...
%             keyboard
        ...
        if cntr > cntrlimit
            warning('YODER: Just FYI, convergence not met before iteration limit.');
            exitFlag = 1;
...
if sum([isnan(alfaFlag), isnan(CLFlag), isnan(deFlag)]) < 2
    error('YODER: Hey! Only choose one of [alfa, CL, or de] to uncomment!.');
end
