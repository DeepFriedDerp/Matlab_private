...
if isempty(varargin) == 1
    ...
    % runfile
    % runfile = 'runfile_main';         % main file, uses CL = 0.5
    runfile = 'runfile_main_v2';        % main file, uses CL = 0.1
else
...
eval(runfile);
fprintf('Damping ratio: %0.6E\n', dr);
fprintf('Damping constant: %0.6E\n', ct);
...
tic
infostruct = PostProcess(runfile, EQNS, OCBm, tout, outsB, QCBm, OvBO_B, F);
t5 = toc;
...
syms Tx Ty Tz
syms MT g Fax Fay Faz Max May Maz
...
% inertia tensor
I_B_Q = MakeMat(Ixx, Iyy, Izz, Ixy, 0, 0);
I_B_B = BCQ*I_B_Q*QCB;
...
function [infoout] = PostProcess(runfile, EQNS, OCBm, tin, outsB, QCBm, OvBO_B, F)
...
momcoe = Tens_B;
for i1 = 1:length(tin)
    xd = statesFile(tin(i1), outsB(i1, :), runfile, EQNS, OCBm, 1, QCBm, OvBO_B, F);
    Tens_B(:, i1) = xd.Tens_B;
...
function [xd] = statesFile(t, x0, runfile, EQNS, OCBm, outflag, QCBm, OvBO_B, F)
...
% vrel_Q = QCO*v_inertial_O - QCB*O_V_BO_B;       % m/s, relative wind
% vrel_B = BCO*v_inertial_O - O_V_BO_B;       % m/s, relative wind - old
vrel_B = BCO*v_inertial_O - [uBO; vBO; wBO];    % m/s, Etkin definition
...
% BCS = [+VE alfa about jB] * [-VE beta about kB]
R_Y_alfa = MakeMatrix('y', alfa);
R_Z_beta = MakeMatrix('z', -beta);
BCS = R_Y_alfa*R_Z_beta;
Faero_B = BCS*Faero_S;
Fax = Faero_B(1);
Fay = Faero_B(2);
Faz = Faero_B(3);
...
% moments
% ---------------------------------------------------------------
% tension moment already included into EOMs
% calculate here for use in post processing
MT_B = CrossMe(rPB_B, T_B);

% aero moments in B frame
% Max = 0;
% May = 0;
% Maz = 0;
Max = Maero_B(1);
...
if segs*n < length(tin)
    plot(outsO(1, 2), outsO(1, 1), 'kx', 'DisplayName', 'Start');
    set(gca, 'ColorOrderIndex', 1);
    in1 = 1;
    in2 = in1 + n - 1;
    for i1 = 1:segs
        plot(outsO(in1:in2, 2), outsO(in1:in2, 1), '-', 'Marker', '.', 'DisplayName', sprintf('t=%0.4fs', tin(in1)));
        in1 = in2;
        in2 = in1 + n;
        if i1 == segs - 1
            in2 = length(tin);
    ...
    plot(outsO(1, 2), outsO(1, 1), 'kx');
    plot(outsO(:, 2), outsO(:, 1), '-.');
end
plot(outsO(end, 2), outsO(end, 1), 'ko', 'DisplayName', sprintf('t=%0.4fs', tin(end)));
xlabel('$j_O$ [m]');
ylabel('$i_O$ [m]');
grid on
