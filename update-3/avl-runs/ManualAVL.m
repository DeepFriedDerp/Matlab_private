...
% % set the directory for avl
% % assume avl is at the highest directory and only needs to enter subs for
% %   other run files
% a1 = strsplit(pwd, '\');
% avldir = [];
% for i1 = 1:length(a1) - 1
%     avldir = fullfile(avldir, a1{i1});
% end
...
newdir = 'hershey';
% avlfile = 'hershey.avl';
avlfile = 'hershey_2.avl';
m = 1.991;
a = 'a c 0.1';  % set alfa so CL is 0.1
b = 'b b 5';    % set beta to zero
% e = 'd1 pm 0';  % de so pitching moment is zero (trim)
% r = 'r p 0';
% p = 'p q 0';
% y = 'y r 0';

% write run file
% assume in the same hershey directory
t1 = strsplit(avlfile, '.');
% basefile = t1{1};
basefile = [t1{1}, 'b5d'];
...
% run it!
cmds = sprintf('%s < %s', 'avl.exe', runfile);
status = system(cmds);